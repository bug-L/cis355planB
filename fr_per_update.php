...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	$title =  $_POST['title'];
	$maddress = htmlspecialchars($_POST['maddress']);
	...
	if (empty($maddress)) {
		$maddressError = 'Please enter a valid address';
		$valid = false;
	}
	if (empty($city)) {
	...
	if (empty($state)) {
		$stateError = 'Please enter a valid state (e.g. MI,WA,NC)';
		$valid = false;
	}
	...
	if ($valid) { // if valid user input update the database
		...
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$sql = "UPDATE fr_persons  set fname = ?, lname = ?, email = ?, mobile = ?, password = ?, title = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ?, maddress = ?, city = ?, state = ?, zip = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $email, $mobile, $passwordhash, $title, $fileName,$fileSize,$fileType,$content, $maddress, $city, $state, $zip, $id));
			Database::disconnect();
    ...
    <div class="container">
		...
		<div class="span10 offset1">
			...
			<form class="form-horizontal" action="fr_per_update.php?id=<?php echo $id?>" method="post" enctype="multipart/form-data">
				...
				<div class="control-group <?php echo !empty($maddressError)?'error':'';?>">
					<label class="control-label">Mailing Address</label>
					<div class="controls">
						<input name="maddress" type="text"  placeholder="Mailing Address" value="<?php echo !empty($maddress)?$maddress:'';?>">
				...
				<div class="control-group <?php echo !empty($stateError)?'error':'';?>">
					...
					<div class="controls">
						<input name="state" type="text"  placeholder="State" minlength="2" maxlength="2" value="<?php echo !empty($state)?$state:'';?>">
				...
				<div class="control-group <?php echo !empty($zipError)?'error':'';?>">
					...
					<div class="controls">
						<input name="zip" type="text" minlength="5" maxlength="5" placeholder="Zip Code" value="<?php echo !empty($zip)?$zip:'';?>">
